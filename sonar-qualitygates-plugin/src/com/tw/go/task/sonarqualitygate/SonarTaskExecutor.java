...
public class SonarTaskExecutor extends TaskExecutor {
	...
	public Result execute() throws Exception {
		...
		String sonarProjectKey = (String) ((Map) this.config.get(SonarScanTask.SONAR_PROJECT_KEY))
				.get(GoApiConstants.PROPERTY_NAME_VALUE);
		log("checking quality gate result for: " + sonarProjectKey);

		try {
			// get input parameter
			String stageName = (String) ((Map) this.config.get(SonarScanTask.STAGE_NAME))
					.get(GoApiConstants.PROPERTY_NAME_VALUE);
			String jobName = (String) ((Map) this.config.get(SonarScanTask.JOB_NAME))
					.get(GoApiConstants.PROPERTY_NAME_VALUE);
			String jobCounter = (String) ((Map) this.config.get(SonarScanTask.JOB_COUNTER))
					.get(GoApiConstants.PROPERTY_NAME_VALUE);

			String sonarApiUrl = (String) ((Map) this.config.get(SonarScanTask.SONAR_API_URL))
					.get(GoApiConstants.PROPERTY_NAME_VALUE);
			log("API Url: " + sonarApiUrl);
			String issueTypeFail = (String) ((Map) this.config.get(SonarScanTask.ISSUE_TYPE_FAIL))
					.get(GoApiConstants.PROPERTY_NAME_VALUE);
			...
			if (envVars.get("SONAR_USER") != null
					&& envVars.get("SONAR_PASSWORD") != null) {

				sonarClient.setBasicAuthentication(envVars.get("SONAR_USER").toString(),
						envVars.get("SONAR_PASSWORD").toString());
				log("Logged in as '" + envVars.get("SONAR_USER").toString()
						+ "' to get the project's quality gate");
			} else {
